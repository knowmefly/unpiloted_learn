...
# param  input-input  kernel-weights biases-bias  k_h/kw 滤波器尺寸  sh/sw  strides  c_o输出深度, group-卷积计算分几组完成（应该是内存不够的情况才这么搞）
def conv(input, kernel, biases, k_h, k_w, c_o, s_h, s_w, padding="VALID", group=1):
    ...
    # Param： i-input  k-weights  []-strides
    ...
    if tf.__version__ < "1.0.0":
        ...
        if group == 1:
            ...
            # 把input这个tensor，在axis=3上（input最后一个维度是通道数量），分成group的形状，这里就是分成一维的
